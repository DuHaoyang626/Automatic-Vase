...
#define LCD1602_DATAPORT P0//LCD数据端口
sbit DIN  = P0^4;	  //ADC输入
sbit CS   = P0^5;	  //ADC片选
sbit CLK  = P0^6;	  //ADC时钟
sbit DOUT = P0^7;	  //ADC输出
...
sbit STEPMOTOR1=P3^0;//步进电机1接入P3^0，待驱动板
sbit STEPMOTOR2=P3^1;//步进电机2接入P3^1，待驱动板
sbit STEPMOTOR3=P3^2;//步进电机3接入P3^2，待驱动板
sbit STEPMOTOR4=P3^3;//步进电机4接入P3^3，待驱动板

// 
// sbit MOISTURE_SOIL=P3^1;//土壤湿度传感器接入ADC
// sbit WATER_LEVEL=P3^2;//水位传感器接入ADC^E4
//sbit LIGHT_INTENSITY=P2^4;//光照强度传感器接入ADC^A4
//sbit WIFI_IO=P2^0;//预留WIFI接口为？？？

//全局数据

//LCD1602数据口4位和8位定义，若为1，则为LCD1602四位数据口驱动，反之为8位
#define LCD1602_4OR8_DATA_INTERFACE	1	//默认使用8位数据口LCD1602

// 定义步进电机速度，值越小，速度越快
#define STEPMOTOR_MAXSPEED        100//步进电机最大速度，不能小于1
#define STEPMOTOR_MINSPEED        350//步进电机最小速度
#define STEPMOTOR_SHIELD_HIGHT    10//遮光板高度
#define STEPMOTOR_SHIELD_RANGE    300//遮光板移动时间比例
u16 StepMotor_status=0;//步进电机遮光板状态变量，0为最低位置，1为最高位置
	...
	while(1)
	{
		...
		stay_time++;
		 if(stay_time==22000){
			...
			adc_light_value=xpt2046_read_adc_value(0xA4);//光敏电阻
			...
			adc_water_level_value=xpt2046_read_adc_value(0xE4);//水位传感器
		...
		switch(key)
		{
			case(1):{
				...
				lcd1602_show_string(0,1,tempbuf);//第二行显示
				break;	
			}
			case(2):{
				lcd1602_init();
				lcd1602_show_string(0,0,"Soil Moisture:");//第一行显示
				lcd1602_show_string(0,1,"N/A");//第二行显示
				break;	
			}
			case(3):{
				lcd1602_init();
				lcd1602_show_string(0,0,"Water Level:");//第一行显示
				lcd1602_show_string(0,1,adc_water_level_buf);//第二行显示
				break;	
			}
			case(4):{
				lcd1602_init();
			...
			case(5):{
				...
				lcd1602_show_string(0,1,adc_light_buf);//第二行显示
				break;	
			}
			case(6):{
				lcd1602_init();
